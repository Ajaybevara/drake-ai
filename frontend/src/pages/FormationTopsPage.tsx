...
async function saveProjectResultCopy(predictionName: string, resultPayload: any) {
  try {
    ...
    useStore.getState().setEnterpriseProject(data.project)
    const projectWithLiveState = await saveModuleViewStateToLocalProject(data.project, 'formationTops', {
      ...getFormationTopsProjectState(),
...
function FormationTopsWorkspace() {
  ...
  async function run() {
    ...
    try {
      ...
      setResult(data)
      await saveProjectResultCopy('formation_tops_detection', data)
  ...
  const browseLas = () => {
    ...
    input.onchange = event => uploadLog((event.target as HTMLInputElement).files?.[0])
    input.click()
  }
